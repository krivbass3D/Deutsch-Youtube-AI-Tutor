...
const LessonView: React.FC<LessonViewProps> = ({ 
  ...
  if (!selectedLesson || !state || !state.progress) return null;
  const { progress } = state;

  return (
    <div className="bg-white rounded-2xl md:rounded-[32px] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden animate-fade-in">
      {/* Lesson Header */}
      <div className="p-4 md:p-8 md:pb-4">
        <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4 md:mb-6">
          <div className="flex-1">
            <h3 className="text-xl md:text-3xl font-black text-slate-800 leading-tight mb-2 tracking-tight">
              {selectedLesson.title}
            </h3>
            <div className="flex items-center gap-2 md:gap-3">
              <span className="px-2 md:px-3 py-0.5 md:py-1 bg-blue-50 text-blue-600 rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-wider">
          ...
          <div className="flex p-1 bg-slate-100 rounded-2xl shrink-0 w-full md:w-auto">
            <button
              onClick={() => setShowStatistics('lesson')}
              className={`flex-1 md:flex-none px-3 md:px-4 py-1.5 md:py-2 rounded-xl text-xs md:text-sm font-bold transition-all ${
